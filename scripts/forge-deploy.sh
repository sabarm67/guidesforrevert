...
# committed pre-built directly into public/ (this server has no Flutter
# SDK, so `flutter build web` can't run here) and served as plain static
# files at the site root, https://guides.rcaquacycle.com/ — routes/web.php
# has a GET / route serving public/index.html as a safety net regardless
# of nginx's index-file precedence. See docs/setup/dev-setup.md for the
