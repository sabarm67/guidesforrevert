...
| `learning_stages`, `lessons`, `lesson_progress` | **Implemented + seeded** — 74 lessons total: the 4-stage linear Learning Journey (6+8+8+9=31 lessons, Stage 1 now closes with "The Five Pillars of Islam" and "The Six Articles of Faith", Stage 2 now includes an "Understanding the Azan" lesson ahead of simplified Wudu/Salah overviews) plus three standalone, non-linear topic collections distinguished by `learning_stages.collection_type` — Fiqh in Daily Life (9 lessons), Understanding Islam: Addressing Misconceptions (31 lessons), and Prayer Guide (3 lessons: full Wudu/Salah step-by-step guides plus "Types of Prayer", surfaced on the Prayer tab rather than the Learning tab). All collections reuse the exact same `lessons`/body-block structure, just outside the linear stage progression — see system-architecture.md |
